    ...
    <main>
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                echo "<tr>";
                echo "<td>{$row['id']}</td><td>{$row['name']}</td><td>{$row['email']}</td><td>{$row['city']}</td><td>{$row['phone']}</td><td>{$row['role']}</td>";
        ...
        if (isset($_GET['edit'])) {
            ...
            <form action="admin_users.php" method="POST">
                ...
                <label>Phone:</label><input type="tel" name="phone" value="<?php echo $user['phone']; ?>"><br>
